...
class EasyPayClient extends EasyPayConfig
{
    /**
     ...
     * @param array  $headers
     *
     * @throws \Artistas\EasyPay\EasyPayException
     *
     ...
     * @return \SimpleXMLElement
     */
    protected function sendJsonTransaction(array $parameters, $url = null, $method = 'POST', array $headers = ['Accept: application/vnd.EasyPay.com.br.v3+json;charset=ISO-8859-1', 'Content-Type: application/json; charset=UTF-8'])
    {
        if ($url === null) {
            $url = $this->url['transactions'];
        }
    ...
    private function executeCurl($parameters, $url, array $headers, $method)
    {
        ...
        if (isset($getInfo['http_code']) && $getInfo['http_code'] == '503') {
            ...
            throw new EasyPayException('Serviço em manutenção.', 1000);
        }
        if ($result === false) {
            $this->log->error('Erro ao enviar a transação', ['Retorno:' => $result]);

            throw new EasyPayException(curl_error($curl), curl_errno($curl));
        }
    ...
    private function formatResult($result)
    {
        if ($result === 'Unauthorized' || $result === 'Forbidden') {
            ...
            throw new EasyPayException($result.': Não foi possível estabelecer uma conexão com o EasyPay.', 1001);
        }
        if ($result === 'Not Found') {
            $this->log->error('Notificação/Transação não encontrada', ['Retorno:' => $result]);

            throw new EasyPayException($result.': Não foi possível encontrar a notificação/transação no EasyPay.', 1002);
        }
        ...
        if (isset($result->error) && isset($result->error->message)) {
            ...
            throw new EasyPayException($result->error->message, (int) $result->error->code);
        }
    ...
    private function formatResultJson($result)
    {
        ...
        if (isset($result->error) && $result->error === true) {
            ...
            throw new EasyPayException($message, (int) $code);
        }
    ...
    protected function validate($data, $rules)
    {
        ...
        if ($validator->fails()) {
            throw new EasyPayException($validator->messages()->first(), 1003);
        }
    }
